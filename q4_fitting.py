...
def theoretical_model(t, L1, L2):
    """
    Theoretical model derived from the linearised SEIR ODE system.
    y(t) = (e^{L1*t} - e^{L2*t}) / (L1 - L2)
    Observable: y(t)^2
    """
...
def exp_model(t, a, b, c):
    """Phenomenological exponential: g(t) = a + b*e^{c*t}"""
...
def main():
    ...
    # --- 2. Theoretical SEIR fit (nonlinear least squares) ---
    # L1 > 0 (growing mode), L2 < 0 (decaying mode)
    ...
    print(f"sigma + gamma       = {-(L1+L2):.4f}   [identifiable]")
    print(f"sigma*(gamma-beta)  = {L1*L2:.4f}   [identifiable]")
    print(f"Doubling time       = {np.log(2)/L1:.3f} time units")
    print()

    # --- 3. Exponential fit (nonlinear least squares) ---
    ...
    print(f"Note: c/2 = {c_e/2:.4f}  ~  Lambda_1 = {L1:.4f}  (expected for large t)")
    print()

    # --- 4. Polynomial fit (linear least squares via normal equations) ---
    degree = 3
    n      = len(t)

    # Build Vandermonde matrix A: rows are [t^3, t^2, t^1, t^0]
    A = np.column_stack([t**k for k in range(degree, -1, -1)])

    # Solve normal equations: (A^T A) x = A^T b
    ATA  = A.T @ A
    ATb  = A.T @ y2
    coeffs = np.linalg.solve(ATA, ATb)   # more stable than explicit inverse

    print(f"=== Polynomial Fit (Degree {degree}) ===")
    print("Solved via normal equations  (A^T A) x = A^T b")
    labels = [f"c_{degree-i}" for i in range(degree+1)]
    for lbl, val in zip(labels, coeffs):
        print(f"  {lbl} = {val:.6f}")
    print()
    ...
    # --- 5. Residual summary ---
    def rmse(pred): return np.sqrt(np.mean((pred - y2)**2))
    ...
    print("Plot saved as q4_model_fits.png")
